...
*   - \b Sensor: High-resolution images, but slower capture rate.
...
* - <b>Imaging targets</b> (if the short-range scan profile was used) and <b>Sensor targets</b>
* (if one of the sensor scan profiles was used): Provide a list of and the number of identified targets
...
* transmitted from antenna <i>i, reflected from the target and recieved in antenna <i>j.
*
...
* (ranges and resolution on the x, y, z axes); If one of the Sensor profiles was used, the arena is usually much larger and \n
...
*
* The scan profile can be changed at any stage, upon which Walabot automatically performs Stop > change profile > Start.
* Upon a profile change, Walabot should be recalibrated.
*
...
*    - For scans that used one of the Sensor profiles, set \b arena by spherical coordinates, with arena resolution:
...
*    - <b>Get action</b>: Walabot_GetRawImage(), Walabot_GetRawImageSlice(), Walabot_GetImageEnergy(),Walabot_GetImagingTargets(),Walabot_GetSensorTargets()
...
* - For detailed error information: Walabot_GetExtendedError()
* - To set the location of Walabot internal database: Walabot_SetSettingsFolder()
...
* <b> C# Sample Code </b>
* \include SensorApp_SampleCode/Program.cs
* <b> C++ Sample Code </b>
* \include Sensor_SampleCode_cpp/SensorSampleCode.cpp
* \n
* -# \b Breathing example, setting up a Narrow Sensor profile
* \n<b> C# Sample Code </b>
* \include "BreathingApp_SampleCode/Program.cs"
* <b> C++ Sample Code </b>
* \include BreathingApp_SampleCode_cpp/SensorBreathingSampleCode.cpp
* \n
* -# \b InWall application  sensor
* \n<b> C# Sample Code </b>
* \include "InWallApp_SampleCode\Program.cs"
* <b> C++ Sample Code </b>
* \include InWallApp_SampleCode_cpp/InWallSampleCode.cpp
* \n
...
* \include FlipPagesSampleCode.cpp
...
* <a href="http://www.walabot.com/gettingstarted"> Walabot Installer </a>
* </b>
...
* The Python API is using the ‘low-level’ library for C++ (WalabotAPIWindows.dll on Windows and libWalabotAPI.so on Linux).\n\n
...
* The Python module will raise WalabotError in case it encounters one. You can catch those errors and act accordingly. A table of the possibles WalabotError’s is given below.
* @subsection _walaboterror WalabotError fields
* message: A short generic description of the occurred error.
* code: The occurred error code, can simplify the debugging process.
* extended: A code used internally, for Walabot developers.
* Available Errors And Codes
...
* Walabot Installer install the libraries in the following folder: "%Program Files%\Walabot\WalabotSDK\bin\Win32" \n
* <b>Building Application using visual studio 2015 C++</b> 
* For C++ project you need to use the following libraries: \n
* -#	WalabotAPIWindows.dll
* -#	WalabotAPIWindows.lib
* 
* <b>Flow</b>: 
* -# In the Visual Studio open new project \n
												 
\image html ProjectSlide1.jpg

* -# Chose console application. \n
* You may chose a folder and project name\n
* Press "OK" and "Finish" \n

* \image html ProjectSlide2a.png
* 
* -# Add libraries into the project
* In the Visual Studio menu: View->Property Manager \n\n
* \image html ProjectSlide4.png
* 
* -# Set <a>WalabotAPIWindows.lib</a> into Linker->Input->Additional Dependency  \n\n
* \image html ProjectSlide5.png
* 
* -# Set <a>"c:/Program Files/walabot/bin/Win32"</a> In the Linker-> General->Additional  Library Directories 
* \image html ProjectSlide7.png
* 
* 
* <b>Building Application using visual studio 2015 C#</b>
* For C# project you need to use the following libraries: \n
...
typedef enum WALABOT_RESULT_ENUM
{
	...
	WALABOT_ERR_INVALID_PORT_DEFINITION,			//!< Invalid antenna number
	WALABOT_ERR_TOO_MANY_SAMPLES,					//!< General Walabot error
	WALABOT_ERR_RECORDING_THREAD_ALREADY_RUNNING,	//!< Application already running
	WALABOT_ERR_RECORDING_WITHOUT_APPLY_SETTINGS,	//!< Walabot_Start() was not called
	WALABOT_ERR_READ_WITHOUT_APPLY_SETTINGS,		//!< Walabot_Start() was not called
	WALABOT_ERR_INVALID_PAGE_INDEX,					//!< General Walabot error
	WALABOT_ERR_INVALID_TXRX_PAIR,					//!< {Tx,Rx} Antenna  pair error
	WALABOT_ERR_AMBIGUOUS_TXRX_PAIR,				//!< The requested tx-rx pair is ambiguous (recorded in more than one port group)
	WALABOT_ERR_PORTGROUP_INDEX_OUT_OF_BOUNDS,		//!< General Walabot error
	WALABOT_ERR_RX_PORT_OUT_OF_BOUNDS,				//!< Rx antenna invalid
	WALABOT_ERR_NO_DATA,							//!< No data returned for specified request
	WALABOT_ERR_USB_READ_FAILURE,					//!< Walabot device disconnected  
	WALABOT_PARAMETER_NOT_FOUND,					//!< Invalid input parameter
	WALABOT_OUT_OF_RANGE,							//!< A provided parameter value was out of its allowed range
	WALABOT_READONLY_PARAMETER,						//!< General Walabot error
	WALABOT_ERR_ARENA_SIZE_TOO_BIG,					//!< Error in arena resize parameters 
	WALABOT_DD_ERR_USB_DEVICE_WENT_DOWN,			//!< Walabot device USB error
	WALABOT_ERR_INVALID_PARAMETER,					//!< A provided parameter value was out of its allowed range
	WALABOT_INSTRUMENT_NOT_FOUND,					//!< The SDK is unable to communicate with the instrument
	WALABOT_BUFFER_TOO_SMALL,						//!< Provided buffer is too small
	WALABOT_INVALID_ARRAY_SIZE,						//!< Provided buffer is too small
	WALABOT_INVALID_PROFILE,						//!< Invalid Profile type
	WALABOT_ERR_NO_IMAGE,							//!< No image has been recored by the Walabot
	WALABOT_ERR_GENERAL,							//!< General Walabot error
	// Here are the error groups
	WALABOT_ERR_PROTOCOL = 0x00010000,				//!< Internal Error
	WALABOT_ERR_DEVICE_STORAGE = 0x00020000,		//!< Unable to write to device storge
	WALABOT_ERR_DB = 0x00030000,					//!< Unable to access Database
	WALABOT_ERR_USB = 0x00040000,					//!< USB 
	WALABOT_ERR_SEQ = 0x00050000,
	
	WALABOT_ERR_SCHEDULER = 0x00070000,				//!< Data recording fetch 
	WALABOT_ERR_HW_PLAN = 0x00080000,				//!< Error in recoding schema 
	WALABOT_ERR_MULTI_CHIP = 0x00090000,			//!< Error in multi Walabot configuration setup 
	WALABOT_ERR_REC_PLAN = 0x000A0000,				//!< Error in recoding schema 
	WALABOT_ERR_NULL_OBJECT = 0x000B0000,			//!< Non initlizated object
	WALABOT_ERR_IN_COMPILER = 0x000C0000,			//!< Error during building recording profile schema
	WALABOT_ERR_IN_INITIALIZATION = 0x000D0000,		//!< Error during initilization process
	WALABOT_ERR_SETTING_FOLDER = 0x000E0000,		//!< Didn't find setting folder. Use Walabot_SetSettingsFolder(...) to fix it
	WALABOT_ERR_LOG = 0x000F0000,					//!< Error during internal logging

	WALABOT_GENERAL_ERROR							//!< A general error occurred. Inspect the error logs for more information
...
{
	PROF_SHORT_RANGE_IMAGING	= 0x00010000,	//!< Short-range, penetrative scanning in dielectric materials
	PROF_SHORT_RANGE_SINGLE_LINE,				//!< Short-range, small number of antennas pairs
...
{
	STATUS_DISCONNECTED, //!< No communication with Walabot
	STATUS_CONNECTED,	//!< Communication established with Walabot, but no Profile set
	STATUS_IDLE,		//!< Profile set, but not currently scanning or calibrating
...
typedef struct SensorTarget_struct //!< Sensor Tagret parameters
{
	...
	#define WALABOT_API extern "C" __declspec(dllexport) 
	#else
	#define WALABOT_API extern "C" __declspec(dllimport)
	#endif
#else
	#define WALABOT_API extern "C"
#endif
...
/**
* @brief Obtains raw image data from specified antennas.
*
